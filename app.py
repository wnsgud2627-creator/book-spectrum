...
# --- [수정 1] 단계별 검색 함수 ---
    ...
    def refine_with_gemini(book_data, title, keyword_pool, category_pool, std_n, total_n, age_group):
        ...
        if "유아" in age_group:
            persona, char_limit = "어린이 교육용 콘텐츠 전문 작가이자 카피라이터", 35
        ...
        [작업 1: 줄거리 요약]
        1. **구조**: [주인공과 상황 도입] → [예기치 못한 사건/문제 발생] → [질문을 통한 호기심 유발]의 3단계 구성을 따를 것(권고).
        2. **서술 방식**: 책의 주제를 설명하지 말고, **책의 첫 장면을 들려주듯 실감 나게** 서술하세요.
        3. **구체성 및 근거**: '음식' 대신 '누룽지'처럼 **[정보 원문]에 명시된 구체적인 단어**를 사용하세요. 
        4. **할루시네이션 방지**: **반드시 제공된 [정보 원문]의 내용 안에서만 작성**하세요. 원문에 없는 인물, 사건, 배경을 상상해서 추가하는 것은 절대 금지입니다.
        ...
        8. **표현 및 문체**: 
           - 다정하고 친근한 경어체(~해요, ~답니다)를 사용하세요.
           - 상상력을 자극하는 의성어/의태어를 적절히 섞고, 어려운 한자어(예: 조우하다)는 쉬운 단어(예: 만나다)로 대체하세요.
