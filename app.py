...
def refine_with_gemini(book_data, title, keyword_pool):
    ...
    prompt = f"""
    당신은 4~7세 도서 추천 시스템의 전문 카피라이터이자 데이터 라벨러입니다.
    '{title}'의 정보를 바탕으로 줄거리와 키워드를 생성하세요.

    [작업 1: 줄거리 요약 - 엄격 규정]
    1. **반드시 딱 3문장**으로만 구성하세요.
    2. **금지어**: "안녕", "친구들", "소개할게요", "이 책은" 등 상투적 표현 절대 금지.
    3. **내용**: 구체적인 소재(주인공, 소품)로 시작하여 담백한 설명 후 제안형으로 마무리하세요.

    [작업 2: 키워드 추출 - 엄격 규정]
    1. **총 5개의 키워드**를 뽑으세요.
    2. **필수 사항**: 아래 제공된 [표준 목록]에서 **반드시 3개**를 먼저 선택하세요.
    3. **추가 사항**: 책의 고유한 소재나 특징을 잘 나타내는 키워드를 **본문에서 2개** 선택하세요.
